                     ...
                     void *spd_state, UWord8 frame_type)
{
   ...
   if(mode <= MODE_7k)
   {
      ...
      ind[3] = *prms++;
      ind[4] = *prms++;
      D_LPC_isf_2s3s_decode(ind, isf, st->mem_isf_q, st->mem_isf, st->mem_isf_buf, bfi);
   }
   else
   {
      ind[0] = *prms++;
   ...
   for(i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
   {
      ...
      D_ACELP_phase_dispersion(gain_code, gain_pit, code, (Word16)j, st->mem_ph_disp);
      ...
      L_gain_code = D_UTIL_mpy_32_16(gain_code, gain_code_lo, (Word16)(32767 - fac));
      D_UTIL_l_extract(L_tmp, &gain_code, &gain_code_lo);
      L_gain_code = L_gain_code + D_UTIL_mpy_32_16(gain_code, gain_code_lo, fac);
      ...
      {
         corr_gain = *prms++;
         D_UTIL_dec_synthesis(p_Aq, exc2, Q_new, &synth16k[i_subfr * 5 / 4], corr_gain, HfIsf, mode, newDTXState, bfi, st);
      }
      else
      {
         D_UTIL_dec_synthesis(p_Aq, exc2, Q_new, &synth16k[i_subfr * 5 / 4], 0, HfIsf, mode, newDTXState, bfi, st);
      }

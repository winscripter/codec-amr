...
void E_DTX_buffer(E_DTX_State *st, Float32 isf_new[], Float32 enr, Word16 codec_mode)
{
   Float32 log_en;
...
static void E_DTX_isf_q(Float32 *isf, Word16 **indice)
{
   ...
   (*indice)[0] = E_LPC_isf_sub_vq(&isf[0], E_ROM_dico1_isf_noise, 2, SIZE_BK_NOISE1, &tmp);
   (*indice)[1] = E_LPC_isf_sub_vq(&isf[2], E_ROM_dico2_isf_noise, 3, SIZE_BK_NOISE2, &tmp);
   (*indice)[2] = E_LPC_isf_sub_vq(&isf[5], E_ROM_dico3_isf_noise, 3, SIZE_BK_NOISE3, &tmp);
   (*indice)[3] = E_LPC_isf_sub_vq(&isf[8], E_ROM_dico4_isf_noise, 4, SIZE_BK_NOISE4, &tmp);
   (*indice)[4] = E_LPC_isf_sub_vq(&isf[12], E_ROM_dico5_isf_noise, 4, SIZE_BK_NOISE5, &tmp);
}
...
void E_DTX_tx_handler(E_DTX_State *st, Word32 vad_flag, Word16 *usedMode)
{
   ...
   else
   {
      ...
      else
      {
         ...
         if ((st->mem_dec_ana_elapsed_count + st->mem_dtx_hangover_count) < DTX_ELAPSED_FRAMES_THRESH)
         {
            *usedMode = MRDTX;
         }
      }
...
static void E_DTX_filter_bank(E_DTX_Vad_State *st, Float32 in[], Float32 level[])
{
   ...
   level[11] = E_DTX_level_calculation(tmp_buf, &st->mem_sub_level[11], (FRAME_LEN >> 2) - 48, FRAME_LEN >> 2, 4, 1, 0.25F);
   level[10] = E_DTX_level_calculation(tmp_buf, &st->mem_sub_level[10], (FRAME_LEN >> 3) - 24, FRAME_LEN >> 3, 8, 7, 0.5F);
   level[9] = E_DTX_level_calculation(tmp_buf, &st->mem_sub_level[9], (FRAME_LEN >> 3) - 24, FRAME_LEN >> 3, 8, 3, 0.5F);
   level[8] = E_DTX_level_calculation(tmp_buf, &st->mem_sub_level[8], (FRAME_LEN >> 3) - 24, FRAME_LEN >> 3, 8, 2, 0.5F);
   level[7] = E_DTX_level_calculation(tmp_buf, &st->mem_sub_level[7], (FRAME_LEN >> 4) - 12, FRAME_LEN >> 4, 16, 14, 1.0F);
   level[6] = E_DTX_level_calculation(tmp_buf, &st->mem_sub_level[6], (FRAME_LEN >> 4) - 12, FRAME_LEN >> 4, 16, 6, 1.0F);
   level[5] = E_DTX_level_calculation(tmp_buf, &st->mem_sub_level[5], (FRAME_LEN >> 4) - 12, FRAME_LEN >> 4, 16, 4, 1.0F);
   level[4] = E_DTX_level_calculation(tmp_buf, &st->mem_sub_level[4], (FRAME_LEN >> 4) - 12, FRAME_LEN >> 4, 16, 12, 1.0F);
   level[3] = E_DTX_level_calculation(tmp_buf, &st->mem_sub_level[3], (FRAME_LEN >> 5) - 6, FRAME_LEN >> 5, 32, 8, 2.0F);
   level[2] = E_DTX_level_calculation(tmp_buf, &st->mem_sub_level[2], (FRAME_LEN >> 5) - 6, FRAME_LEN >> 5, 32, 24, 2.0F);
   level[1] = E_DTX_level_calculation(tmp_buf, &st->mem_sub_level[1], (FRAME_LEN >> 5) - 6, FRAME_LEN >> 5, 32, 16, 2.0F);
   level[0] = E_DTX_level_calculation(tmp_buf, &st->mem_sub_level[0], (FRAME_LEN >> 5) - 6, FRAME_LEN >> 5, 32, 0, 2.0F);
}
...
static void E_DTX_speech_estimate(E_DTX_Vad_State *st, Float32 in_level)
{
   ...
   if (((st->mem_vadreg & 0x4000) || (in_level > st->mem_speech_level)) && (in_level > MIN_SPEECH_LEVEL1))
   {

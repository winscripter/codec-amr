...
void E_MAIN_reset(void *st, Word16 reset_all)
{
   ...
   if (reset_all != 0)
   {
      ...
      for (i = 0; i < M - 1; i++)
      {
         cod_state->mem_isp[i] = (Float32)cos(3.141592654 * (Float32)(i + 1) / (Float32)M);
      }
                    ...
                    void *spe_state, Word16 allow_dtx)
{
   ...
   E_LPC_lag_wind(r + 1, M);
   E_LPC_lev_dur(A, r, M);
   E_LPC_a_isp_conversion(A, ispnew, st->mem_isp, M);
   ...
   if(*mode == MRDTX)
   {
      ...
      memcpy(st->mem_speech, &st->mem_speech[L_FRAME], (L_TOTAL - L_FRAME) * sizeof(Float32));
      memcpy(st->mem_wsp, &st->mem_wsp[L_FRAME / OPL_DECIM], (PIT_MAX / OPL_DECIM) * sizeof(Float32));
   ...
   for (i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR)
   {
      ...
      if (*mode <= MODE_9k)
      {
          index = (Word16)E_ACELP_gains_quantise(s_code, 6, gain_pit, &s_gain_pit, &L_gain_code, g_coeff, clip_gain, st->mem_gain_q);

         E_MAIN_parm_store(index, &prms);
      }
      else
      {
         index = (Word16)E_ACELP_gains_quantise(s_code, 7, gain_pit, &s_gain_pit, &L_gain_code, g_coeff, clip_gain, st->mem_gain_q);
         E_MAIN_parm_store(index, &prms);
      }
      ...
      if (Q_new > st->mem_q)
      {
         E_UTIL_signal_up_scale(exc + i_subfr - (PIT_MAX + L_INTERPOL), (Word16)(Q_new - st->mem_q));
      }
      else
      {
         E_UTIL_signal_down_scale(exc + i_subfr - (PIT_MAX + L_INTERPOL), PIT_MAX + L_INTERPOL + L_SUBFR, (Word16)(st->mem_q - Q_new));
      }
      st->mem_q = (Word16)Q_new;

      E_GAIN_clip_pit_test((Float32)(s_gain_pit * pow(2, -14)), st->mem_gp_clip);
      ...
      for (i = 0; i < L_SUBFR; i++)
      {
         ...
         l_tmp = l_tmp << 5;
         l_tmp += exc[i + i_subfr] * s_gain_pit;
      ...
      if(*mode >= MODE_24k)
      {
         ...
         f_tmp = (Float32)(((fac * f_tmp) + ((1.0 - fac) * (gain_code * pow(2, -st->mem_q)))) * 0.001953125F);
         ...
         {
            f_exc2[i + i_subfr] += f_code[i] - (f_tmp * f_code[i - 1]) - (f_tmp * f_code[i + 1]);
         }

         f_exc2[i_subfr + L_SUBFR - 1] += f_code[L_SUBFR - 1] - (f_tmp * f_code[L_SUBFR - 2]);
         corr_gain = (Word16)E_UTIL_enc_synthesis(p_Aq, &f_exc2[i_subfr], &f_speech16k[i_subfr * 5 /4], st);

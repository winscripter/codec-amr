...
static enum DTXStateType rx_dtx_handler( dtx_decState *st, enum RXFrameType frame_type )
{
   ...
   if ( table_SID[frame_type] | ( ( st->dtxGlobalState != SPEECH ) & table_speech_bad[frame_type] ) ) {
      newState = DTX;
...
static void Lsf_lsp( Word32 lsf[], Word32 lsp[] )
{
   for ( Word32 i = 0; i < M; i++ ) {
      Word32 ind = lsf[i] >> 8;
      Word32 offset = lsf[i] & 0x00ff;
      Word32 tmp = ( ( cos_table[ind+1]-cos_table[ind] )*offset ) << 1;
      ...
      Word32 A_t[] )
{
   ...
   Word16 log_en_index, log_en_int_e, log_en_int_m, level, ma_pred_init, tmp_int_length;
   ...
   if ( ( st->dtxHangoverAdded != 0 ) & ( st->sid_frame != 0 ) ) {
      ...
      for ( i = 0; i < M; i++ ) {
         ...
         for ( j = 0; j < 8; j++ ) {
            ...
            negative = st->lsf_hist_mean[i + j * M] < 0 ? 1 : 0;
...
static void Int_lpc_1and3( Word32 lsp_old[], Word32 lsp_mid[], Word32 lsp_new[], Word32 Az[] )
{
   Word32 lsp[M];
   Word32 i;
...
static void gc_pred( gc_predState *st, enum ModeNB mode, Word32 *code, Word32 *exp_gcode0, Word32 *frac_gcode0, Word32 *exp_en, Word32 *frac_en )
{
...
static void gc_pred_average_limited( gc_predState *st, Word32 *ener_avg_MR122, Word32 *ener_avg )
{
   Word32 av_pred_en, i;
...
static void ec_gain_code( ec_gain_codeState *st, gc_predState *pred_state, Word16 state, Word32 *gain_code )
{
...
static void ec_gain_code_update( ec_gain_codeState *st, Word16 bfi, Word16 prev_bf, Word32 *gain_code )
{
   if ( bfi == 0 ) {
      if ( prev_bf != 0 ) {
...
static void agc2( Word32 *sig_in, Word32 *sig_out )
{
   ...
   if ( s == 0 ) {
      ...
      gain_in = s < 0x7fff7fff ? ( Word16 )( ( s + 0x00008000L ) >> 16 ) : 32767;
      exp = ( exp - i );
...
static Word16 Bgn_scd( Bgn_scdState *st, Word32 ltpGainHist[], Word32 speech[], Word32 *voicedHangover )
{
   Word32 temp, ltpLimit, frame_energyMin, currEnergy, noiseFloor, maxEnergy, maxEnergyLastPart, s, i;
   ...
   currEnergy = ((s < 0xFFFFFFF) & (s >= 0)) ? s >> 13 : 32767;
   ...
   if ( ( maxEnergy> LOWERNOISELIMIT )&( currEnergy<FRAMEENERGYLIMIT )&(
      ...
      st->bgHangover = ( st->bgHangover + 1 ) > 30 ? 30 : st->bgHangover + 1;
   } else {
      st->bgHangover = 0;
   }
   ...
   if ( st->bgHangover > 20 ) {
      prevVoiced = gmed_n( ltpGainHist, 9 ) > ltpLimit ? 1 : 0;
   }
   ...
   if ( prevVoiced ) {
      ...
      *voicedHangover = temp > 10 ? 10 : temp;
   }
   return inbgNoise;
}
...
static void Decoder_amr( Decoder_amrState *st, enum ModeNB mode, Word16 parm[], enum RXFrameType frame_type, Word32 synth[], Word32 A_t[] )
{
   Word32 lsp_new[M];
   Word32 lsp_mid[M];
   Word32 prev_lsf[M];
   ...
   enum DTXStateType newDTXState = rx_dtx_handler( st->dtxDecoderState, frame_type );

   if ( newDTXState != SPEECH ) {
      Decoder_amr_reset( st, MRDTX );
      dtx_dec( st->dtxDecoderState, st->mem_syn, st->lsfState, st->pred_state, st->Cb_gain_averState, newDTXState, mode, parm, synth, A_t );
...
void Speech_Decode_Frame( void *st, enum ModeNB mode, Word16 *parm, enum RXFrameType frame_type, Word16 *synth )
{
...
static int Decoder_amr_init( Decoder_amrState **state )
{
   ...
   *state = NULL;
   ...
   if ( ( s->lsfState = ( D_plsfState * ) malloc( sizeof( D_plsfState ) ) ) == NULL ) {
      return-1;
   }

   if ( ( s->ec_gain_p_st = ( ec_gain_pitchState * ) malloc( sizeof( ec_gain_pitchState ) ) ) == NULL ) {
      return-1;
   }

   if ( ( s->ec_gain_c_st = ( ec_gain_codeState * ) malloc( sizeof( ec_gain_codeState ) ) ) == NULL ) {
      return-1;
   }
   ...
   if ( ( s->Cb_gain_averState = ( Cb_gain_averageState * ) malloc( sizeof( Cb_gain_averageState ) ) ) == NULL ) {
      return-1;
   }
